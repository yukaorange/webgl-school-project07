...
export class Controls {
  ...
  onMouseUp(event) {
    ...
    if (!event.shiftKey && this.picker) {//shift key is not pressed running this code
      // if shift key is pressed, then the user is trying to select multiple objects
      this.picking = false;
      this.picker.stop();
    }
  }
  ...
  onMouseDown(event) {
    ...
    if (!this.picking) this.picker.stop();//if picking is false, then stop picking. Finnally pickedList is to be empty
  }
