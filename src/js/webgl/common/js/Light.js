...
class LightsManager {
  ...
  getArray(type) {
    return this.list.reduce((result, light) => {
      //reduce => result is the return value of last callback
      result = result.concat(light[type]); //concat makes new array that is the combination of result and light[type]
      return result;
    }, []);
  }
