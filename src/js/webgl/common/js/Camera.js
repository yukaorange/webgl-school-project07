...
export class Camera {
  ...
  // Change camera type
  setType(type) {
    ~Camera.TYPES.indexOf(type) // type is existing in Camera this.type = type
      ? //if type is not existing in Camera.TYPES, ~Camera.TYPES.indexOf(type) will return -1
        //-1's bit not is 0 , 0 is false in javascript
        //other number's bit not is 1, 1 is true in javascript
        (this.type = type)
  ...
  goHome(home) {
    ...
    this.setAzimuth(0) //方位角
    this.setElevation(0) //仰角
  }
  ...
  getViewTransform() {
    ...
    mat4.invert(matrix, this.matrix) //invert is getting the inverse matrix.inverse is the opposite of the matrix.
    return matrix
  }
}

// Two defined modes for the camera
Camera.TYPES = ['ORBITING_TYPE', 'TRACKING_TYPE'] //adding static property to Camera class
