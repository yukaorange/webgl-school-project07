...
export const utils = {
  ...
  configureControls(settings, options = { width: 300 }) {
    ...
    const isFolder = v =>
      !isAction(v) &&
      typeof v === 'object' &&
      (v.value === null || v.value === undefined);//checking if the value is Folder

    const isColor = v =>
      (typeof v === 'string' && ~v.indexOf('#')) ||
      (Array.isArray(v) && v.length >= 3);//checking if the value is Color
    ...
    Object.keys(settings).forEach(key => {
      ...
      const {
        ...
        options,
        onChange = () => null,
      } = settingValue;//ex {car:Model:{{options:,onChange:()=>{},value:selectedCar}}} ......Model is the folder.
