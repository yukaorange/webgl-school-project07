...
export const utils = {
  ...
  //   const gui = options.gui || new dat.GUI(options) //options.gui is going to be folder.
  //   const state = {}

  //   const isAction = (v) => typeof v === 'function' // checking if the value is a function

  //   const isFolder = (v) => !isAction(v) && typeof v === 'object' && (v.value === null || v.value === undefined) //checking if the value is Folder

  //   const isColor = (v) => (typeof v === 'string' && ~v.indexOf('#')) || (Array.isArray(v) && v.length >= 3) //checking if the value is Color
  ...
  //       //go inside the folder and check for the values
  //       return utils.configureControls(settingValue, { gui: gui.addFolder(key) })
  //     }

  //     const { value, min, max, step, options, onChange = () => null } = settingValue //ex {car:Model:{{options:,onChange:()=>{},value:selectedCar}}} ......Model is the folder.
  //     //folder is recursively calling the configureControls function and now is the time to check for the values in the folder.
